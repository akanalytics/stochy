    ...
    O: CostFunction<Param = Inp, Output = Out>,
{
    ...
    fn next_iter(
        ...
        let Some(spsa) = self.algo.as_mut() else {
            return Err(Error::msg("SpsaAlgo is not initialized"));
        };

        let mut f = |x: &[f64]| p.cost(&x.to_vec()).unwrap();
        let mut func = FuncKind::FnDyn(&mut f);
        self.control_flow = SpsaAlgo::step_df(&spsa.params, &mut spsa.state, &mut func)
    ...
    fn init(
        ...
        let Some(params) = self.params.take() else {
            return Err(Error::msg("SpsaAlgo params is missing"));
        };
        let spsa = SpsaAlgo::from_fn(params, x0, f).map_err(|e| Error::msg(e.to_string()))?;
        self.algo = Some(spsa);
        Ok((step, None))
    }
