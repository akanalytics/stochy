...
use crate::{RspsaParams, RspsaSolver};
    ...
    // <Self as Algo>::Error : Send + Sync
{
    ...
    fn next_iter(
        ...
        let Some(rspsa) = self.algo.as_mut() else {
            return Err(Error::msg("RspsaSolver is not initialized"));
        };

        let mut f = |x: &[f64]| p.cost(&x.to_vec()).unwrap();
        let mut func = FuncKind::FnDyn(&mut f);
        self.control_flow = RspsaSolver::step_df(&rspsa.params, &mut rspsa.state, &mut func)
    ...
    fn init(
        ...
        let Some(params) = self.params.take() else {
            return Err(Error::msg("RspsaSolver params is missing"));
        };
        let rspsa = RspsaSolver::from_fn(params, &x0, f).map_err(|e| Error::msg(e.to_string()))?;
